class TopicsController < ApplicationController
  def generate
    ...
    return render json: { error: "Enter a topic name." }, status: :unprocessable_entity if subject.blank?
    return render json: { error: "Topic name must be 100 characters or fewer." }, status: :unprocessable_entity if subject.length > 100
